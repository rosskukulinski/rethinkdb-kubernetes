...
if [[ -n "${KUBERNETES_SERVICE_HOST}" && -z "${USE_SERVICE_LOOKUP}" ]]; then
  ...
  # We can just use ${RETHINK_CLUSTER_SERVICE} due to dns lookup
  # Instead though, let's be explicit:
  JOIN_ENDPOINTS=$(getent hosts "${RETHINK_CLUSTER_SERVICE}.${POD_NAMESPACE}.svc.cluster.local" | awk '{print $1}')
